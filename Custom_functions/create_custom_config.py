...
def createVitrolifeConfiguration(FLAGS=Namespace()):
    ...
    # cfg.SOLVER.IMS_PER_BATCH = FLAGS.batch_size if "BATCH_SIZE" in key_list else 4          # Batch size used when training => batch_size pr GPU = batch_size // num_gpus
    # cfg.SOLVER.MAX_ITER = FLAGS.max_iter if "MAX_ITER" in key_list else int(1e1)            # Maximum number of iterations to train for
