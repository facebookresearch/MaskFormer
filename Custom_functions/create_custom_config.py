...
def createVitrolifeConfiguration(FLAGS):
    ...
    if "DATASET_NAME" in key_list:                                                          # If we are choosing a dataset using the command line input arguments ...
        ...
        if "vitrolife" not in FLAGS.dataset_name.lower() and FLAGS.use_checkpoint==True:    # If we instead are using the ADE20K dataset ...
            cfg.MODEL.WEIGHTS = os.path.join(MaskFormer_dir, "maskformer_model_checkpoints",    # ... we can choose whether or not to start with randomly initialized weights ...
