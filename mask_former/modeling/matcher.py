...
class HungarianMatcher(nn.Module):
    ...
    def memory_efficient_forward(self, outputs, targets):
        ...
        for b in range(bs):
            ...
            num_gts = masks.shape[0]
            dtype = masks.dtype
            device = masks.device
            tgt_mask = torch.zeros((num_gts, h_max, w_max), dtype=dtype, device=device)
            # TODO use valid to mask invalid areas due to padding in loss
            valid = torch.ones((num_gts, h_max, w_max), dtype=torch.bool, device=device)

            tgt_mask[:, : masks.shape[1], : masks.shape[2]].copy_(masks)
            valid[:, : masks.shape[1], : masks.shape[2]] = False
