...
def assign_free_gpus(max_gpus=1):
    ...
    reserved_mem_info = list(filter(lambda info: 'Reserved' in info, gpu_info))
    reserved_mem_info = [int(x.split(':')[1].replace('MiB', '').strip()) for x in reserved_mem_info]
    available_mem_info = np.subtract(total_mem_info, reserved_mem_info)
